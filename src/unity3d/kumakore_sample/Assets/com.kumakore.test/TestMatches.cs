...
namespace com.kumakore.test
{
	...
	public class TestMatches : TestBase
    {
        ...
        {
			app().getUser().getOpenMatches().createNewMatch(VALID_RIVAL).sync (delegate(ActionMatchCreate action) {
				Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
				Assert.GreaterOrEqual(app ().getUser().getOpenMatches().Count,1);
			});
        }
        ...
        {
            app().getUser().getOpenMatches().createNewMatch(VALID_RIVAL).async (delegate(ActionMatchCreate action) {
                Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
				Assert.GreaterOrEqual(app ().getUser().getOpenMatches().Count,1);
                Release();
            });
        ...
        {
			app().getUser().getOpenMatches().createRandomMatch().sync (delegate(ActionMatchCreateRandom action) {
				Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
				Assert.GreaterOrEqual(app ().getUser().getOpenMatches().Count,1);
			});
        }
        ...
        {
			List<String> list = new List<String>(app ().getUser().getOpenMatches().Keys);
			foreach(String matchId in list) {
				app().getUser ().getOpenMatches()[matchId].close ().sync (delegate(ActionMatchClose action) {
					Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
				});
			}
			SyncGetOpenMatchList();
			Assert.True (app ().getUser ().getOpenMatches().Count == 0);
        }
        ...
        public void AsyncCloseMatches()
        {
			List<String> list = new List<String>(app ().getUser().getOpenMatches().Keys);
			foreach(String matchId in list) {
				app().getUser ().getOpenMatches()[matchId].close ().async (delegate(ActionMatchClose action) {
					Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
					Release ();
				});
				Wait ();
        ...
        public void SyncResignAll()
        {
			List<String> list = new List<String>(app ().getUser().getOpenMatches().Keys);
			foreach(String matchId in list) {
				app().getUser ().getOpenMatches()[matchId].resign ().sync (delegate(ActionMatchResign action) {
					Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
				});
			}
			SyncGetOpenMatchList();
        ...
        public void AsyncResignAll()
        {
			List<String> list = new List<String>(app ().getUser().getOpenMatches().Keys);
			foreach(String matchId in list) {
				app().getUser ().getOpenMatches()[matchId].resign ().async (delegate(ActionMatchResign action) {
					Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
					Release ();
				});
				Wait ();
        ...
        public void SyncRejectAll()
        {
			List<String> list = new List<String>(app ().getUser().getOpenMatches().Keys);
			foreach(String matchId in list) {
				app().getUser ().getOpenMatches()[matchId].reject ().sync (delegate(ActionMatchReject action) {
					Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
				});
			}
			SyncGetOpenMatchList();
        ...
        public void AsyncRejectAll()
        {
			List<String> list = new List<String>(app ().getUser().getOpenMatches().Keys);
			foreach(String matchId in list) {
				app().getUser ().getOpenMatches()[matchId].reject ().async (delegate(ActionMatchReject action) {
					Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
					Release ();
				});
				Wait ();
        ...
        {
			app().getUser().getOpenMatches().get ().sync (delegate(ActionMatchGetOpen action) {
				Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
			});
        }
        ...
        {
			app().getUser().getClosedMatches().get ().sync (delegate(ActionMatchGetClosed action) {
				Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
			});
        }
        ...
        public void SyncGetStatus()
        {
			List<String> list = new List<String>(app ().getUser().getOpenMatches().Keys);
			foreach(String matchId in list) {
				app().getUser ().getOpenMatches()[matchId].getStatus ().sync (delegate(ActionMatchGetStatus action) {
					Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
				});
			}
			if(app ().getUser ().getOpenMatches().Count == 0) Assert.Pass("No open matches found");
        }
        ...
        public void AsyncGetStatus()
        {
			List<String> list = new List<String>(app ().getUser().getOpenMatches().Keys);
			foreach(String matchId in list) {
				app().getUser ().getOpenMatches()[matchId].getStatus ().async (delegate(ActionMatchGetStatus action) {
					Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
					Release ();
				});
				Wait();
        ...
        {
			foreach(OpenMatch match in app ().getUser().getOpenMatches().Values) {
