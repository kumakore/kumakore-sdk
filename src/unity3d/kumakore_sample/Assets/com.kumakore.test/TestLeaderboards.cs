...
namespace com.kumakore.test
{
	...
	public class TestLeaderboards : TestBase
    {
		...
		public Leaderboard GetLeaderboardByName(String leaderboardName) {
			foreach(Leaderboard l in app1().getLeaderboards().Values) {
        ...
        {
            app1().getLeaderboards().get().sync(delegate(ActionLeaderboardGet action)
            {
        ...
        {
            app1().getLeaderboards().get().async(delegate(ActionLeaderboardGet action)
            {
                Assert.AreEqual(StatusCodes.SUCCESS, action.getCode());
                Release();
            });
